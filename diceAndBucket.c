...
{   int i,j,sum=0,empty_space=0,overflow=0,total_cap=0,volume;
    ...
    for(i=6;i<55;i++){
        if(bucket[i]-volume<0){
            total_cap+=bucket[i];
            empty_space+=volume-bucket[i];
        }
        else if(bucket[i]-volume>0){
            total_cap+=volume;
            overflow+=bucket[i]-volume;
        }
        else
        total_cap+=volume;
    }
    printf("%d\n%d\n%d\n",total_cap,overflow,empty_space);
    return 0;
}